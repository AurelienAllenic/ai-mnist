...
# Définir la classe Net avec ajustements pour compatibilité
class Net(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.pool = nn.MaxPool2d(2, 2, ceil_mode=False)  # Simplifié
        self.fc1 = nn.Linear(64 * 4 * 4, 64)
        self.fc2 = nn.Linear(64, 32)
        self.fc3 = nn.Linear(32, 10)

    def forward(self, x):
        x = self.pool(F.relu(self.bn1(self.conv1(x))))  # Utiliser ReLU
        x = self.pool(F.relu(self.bn2(self.conv2(x))))
        x = torch.flatten(x, 1)
        x = F.relu(self.fc1(x))  # Pas de Dropout
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
        return x
...
# Ajouter cette boucle pour figer BatchNorm
...
# Exporter le modèle vers ONNX avec opset 7
try:
    torch.onnx.export(
        model,
        dummy_input,
        ...
        opset_version=7,  # Utiliser opset 7 pour compatibilité avec onnxjs 0.1.8
